...
- 📈 **Galaxy / AmazingData A-share provider (Phase 1 + 3)** — added `GalaxyFetcher` for查询式 A 股历史日线、证券基础信息与交易日历，并将财务 / 业绩数据通过 Galaxy-first fundamental adapter 接入统一基本面上下文；订阅式实时行情仍留待后续阶段
- 🐳 **Docker Galaxy wheel auto-install** — Docker build now auto-installs any AmazingData / tgw wheels placed under `vendor/galaxy/`, so NAS deployments can enable Galaxy with `git pull + build` instead of re-running manual `pip install` inside containers
- 🟥 **Docker runtime switched to UBI9 Python 3.11 by default** — runtime Docker base image now defaults to `registry.access.redhat.com/ubi9/python-311:latest` and keeps cross-distro package install logic so NAS deployments can validate AmazingData / `tgw` against an environment closer to the vendor's RedHat-oriented Linux recommendation
