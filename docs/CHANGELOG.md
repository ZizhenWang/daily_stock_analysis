...
- 🔐 **Docker/NAS Codex auth reuse fix** — Codex backend now prefers the mounted `CODEX_HOME` when seeding per-call auth state, fixing NAS/Docker cases where `/help` worked but `/analyze` returned placeholder content because `codex exec` could not see the persisted login
