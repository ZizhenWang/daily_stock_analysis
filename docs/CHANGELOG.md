...
- 📋 **Bot watchlist full listing** — `/watchlist list all` now returns the full关注列表 instead of truncating to the default preview window
...
- 🔀 **Docker Codex auth-mode switch** — added `CODEX_AUTH_MODE=shared|account|api` so NAS / Docker deployments can keep ChatGPT-account login and API-key login in separate subdirectories under the same `codex-home/` mount and switch between them from `.env`
- 🔑 **Docker Codex API-key auto login** — when `CODEX_AUTH_MODE=api`, Docker entrypoint now auto-runs `codex login --with-api-key` using `CODEX_OPENAI_API_KEY` (or fallback `OPENAI_API_KEY`) if the target auth directory does not yet contain `auth.json`
- 📈 **Galaxy / AmazingData A-share provider (Phase 1 + 3)** — added `GalaxyFetcher` for查询式 A 股历史日线与证券基础信息，并将财务 / 业绩数据通过 Galaxy-first fundamental adapter 接入统一基本面上下文；订阅式实时行情仍留待后续阶段
- 🌉 **Galaxy bridge mode for NAS** — main application now accesses AmazingData through a remote HTTP bridge instead of importing `AmazingData` / `tgw` inside the NAS main container; this keeps the NAS deployment on the standard Python 3.11 runtime and isolates broker SDK compatibility to a separate host
- 🛠️ **Galaxy bridge scaffold** — added `bridge/galaxy/` with FastAPI app, AmazingData SDK wrapper, smoke-test script, env sample, and systemd service template so Aliyun / Ubuntu hosts can be used as a standalone broker-data bridge for the NAS deployment
- 🔧 **Galaxy bridge Ubuntu template alignment** — updated the `bridge/galaxy` systemd template and README examples to match the current Aliyun deployment path (`/opt/stock-analyzer/daily_stock_analysis`) and `stock` runtime user, and clarified that idle recycle now releases SDK references without explicit logout
- 🧪 **Galaxy SDK probe image** — added `docker/Dockerfile.galaxy-probe` and `docker/docker-compose.galaxy-probe.yml` so NAS deployments can validate `tgw` / `AmazingData` in an isolated runtime (default `ubi7/python-38`, optional `cp311`/other image overrides) without forcing the main application onto Python 3.8
- 🔐 **Docker/NAS Codex auth reuse fix** — Codex backend now prefers the mounted `CODEX_HOME` when seeding per-call auth state, and now copies the full mounted home instead of only `auth.json/config.toml`; this fixes NAS/Docker cases where `/help` worked but `/analyze` or `--llm-smoke-test` still returned placeholder/empty content after switching from account login to API key login
- 🩺 **Codex empty-response diagnostics** — when `codex exec` writes an empty output file, the backend now falls back to `stdout` when possible and includes compact `stdout/stderr` snippets in the raised error, making API key / model / CLI mismatch issues much easier to diagnose
- 🧭 **Codex temp-home path compatibility** — per-call temporary `CODEX_HOME` is now created under a workspace/persistent directory instead of `/tmp`, fixing newer Codex CLI versions that refuse to create helper binaries under temporary directories and would otherwise fail NAS/Docker smoke tests with empty responses
- 🛟 **Codex structured-output fallback** — if `codex exec --output-schema ... -o response.json` still returns an empty output file, the backend now automatically retries once with a plain “return raw JSON only” prompt and parses stdout, improving compatibility with API-key/custom-provider CLI paths that can answer correctly in plain mode but not emit the structured output file
- 🗄️ **SQLite lock mitigation for watchlist APIs** — SQLite connections now enable `WAL + busy_timeout + foreign_keys`, and watchlist CRUD / relation APIs return a clearer `db_locked` response instead of surfacing a generic 500 when concurrent containers briefly contend on the shared database
- 🧵 **DatabaseManager thread-safe initialization** — the SQLite singleton now guards both instance creation and first-time initialization, fixing intermittent NAS/Docker warnings where early concurrent Bot analysis requests could hit `DatabaseManager 未正确初始化`
- 📈 **Futu OpenAPI provider (Phase 1)** — added optional `FutuFetcher` backed by OpenD + `futu-api`, supporting pull-based historical K lines and realtime quotes for `us/hk` by default (optional `cn`), with clean fallback into the existing provider chain
- 📘 **Synology NAS deployment guide** — merged the practical NAS deployment, update commands, Codex-in-Docker login flow, and NAS-specific troubleshooting notes into the top section of `docs/DEPLOY.md`; `docs/synology-nas-deploy.md` now serves as a lightweight redirect
