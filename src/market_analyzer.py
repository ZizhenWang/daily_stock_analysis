...
class MarketAnalyzer:
    ...
    def search_market_news(self) -> List[Dict]:
        ...
        try:
            ...
            market_name = "A股大盘" if self.region == "cn" else "美股大盘"
...
- 大涨: {overview.limit_up_count} 家 | 大跌: {overview.limit_down_count} 家
- 成交额（如有）: {overview.total_amount:.0f}"""
            else:
                stats_block = "## 市场概况\n（美股暂无与 A 股完全等价的涨跌家数统计）"
...
领跌: {bottom_sectors_text if bottom_sectors_text else "暂无数据"}"""
        ...
        data_no_indices_hint = (
            f"注意：由于{market_label}行情数据获取失败，请主要根据【市场新闻】进行定性分析和总结，不要编造具体的指数点位。"
            if not indices_text
            else ""
        )
...
- 必须使用中文输出；若涉及美股指数、ETF、公司名或英文缩写（如 S&P 500、NASDAQ、AAPL），可保留原文，但解释和结论必须是中文
- 若数据不足，请明确写“暂无数据”或“数据不足”，不要编造
