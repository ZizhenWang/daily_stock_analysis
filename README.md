...
>
> 当前 Futu 接入为 **Phase 1 拉取型行情 provider**：
> - 已支持：历史 K 线、实时快照 / 实时报价
> - 未接入：交易接口、新闻资讯、订阅推送流
> - 推荐定位：**港股主行情源**，美股增强源；A 股是否可用取决于 OpenD 当前账号的实际权限
...
> Docker / NAS 场景下如果 `server` 与 `analyzer` 共享同一个 SQLite 数据库，仓库现已默认开启 `WAL + busy_timeout` 来降低瞬时锁冲突；若 watchlist 编辑仍偶发失败，建议稍后重试并查看后端日志中的 `db_locked` 提示。
>
...
> ```
> 登录完成后，`server` 与 `analyzer` 容器会共享 `CODEX_HOME=/codex-home` 中的登录态。若你改成 API key 模式，Codex CLI 官方支持 `codex login --with-api-key`；当前 Docker 入口会在 `CODEX_AUTH_MODE=api` 且存在 `CODEX_OPENAI_API_KEY`（或回退 `OPENAI_API_KEY`）时自动完成一次 API 登录。若你希望账号登录和 API key 登录并存，可在 `.env` 中设置 `CODEX_AUTH_MODE=account|api|shared`，容器会自动把实际登录目录切换到 `/codex-home/account`、`/codex-home/api` 或旧版共享目录 `/codex-home`。
